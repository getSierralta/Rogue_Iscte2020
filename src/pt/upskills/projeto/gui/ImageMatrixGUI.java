 ...
 * Implementations of ImageTile should sync getPosition() and setPosition()
 * 
 * */
...
public class ImageMatrixGUI extends Observable {
	...
	private final List<ImageTile> images = new ArrayList<>();
	private final List<ImageTile> statusImages = new ArrayList<>();
	private final List<ImageTile> statusImages2 = new ArrayList<>();
	...
	private void init() {
		...
		info = new InfoWindow();
		information = new InfoWindow2();
		text = new JLabel("Press Esc to start, press L to load");
		...
		information.setPreferredSize(new Dimension(N_SQUARES_WIDTH * SQUARE_SIZE, SQUARE_SIZE));
	...
	public void setText(JLabel text, boolean flag) {
		information.remove(this.text);
		information.removeAll();
		this.text = text;
		this.text.setFont(new Font("Verdana", Font.PLAIN, 14));
		if (flag){
			this.text.setForeground(Color.GREEN.darker());
		}else{
			this.text.setForeground(Color.RED.darker());
		}
		information.add(this.text);
	...
	public void removeStatusImage2(final ImageTile image) {
		synchronized (statusImages2) { // Added 16-Mar-2016
			statusImages2.remove(image);
		}
	}
	public void addStatusImage2(final ImageTile image) {
		synchronized (statusImages2) { // Added 16-Mar-2016
			statusImages2.add(image);
		}
	}
	...
	public List<ImageTile> getStatusImages() {
		return statusImages;
	}
	...
	private class InfoWindow2 extends JPanel {
		@Override
		public void paintComponent(Graphics g) {
			synchronized (statusImages2) { // Added 16-Mar-2016
				for (ImageTile i : statusImages2)
					g.drawImage(imageDB.get(i.getName()).getImage(), i.getPosition().getX() * SQUARE_SIZE, 0,
							SQUARE_SIZE, SQUARE_SIZE, frame);
